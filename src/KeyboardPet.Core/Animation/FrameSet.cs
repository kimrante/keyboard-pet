...
/// </param>
/// <param name="IdleFrameIndex">
/// 무입력 복귀 시 보여줄 프레임. null이면 루프의 첫 프레임. 루프 밖(키 전용) 프레임이어도 된다.
/// </param>
    ...
    int? IdleFrameIndex = null)
{
    ...
    /// <summary>루프의 첫 프레임 인덱스(세트 전환·규칙 복귀 시 보여줄 프레임).</summary>
    public int FirstLoopIndex => LoopFrames is { Count: > 0 } loop ? loop[0] : 0;

    /// <summary>무입력 복귀 시 보여줄 프레임 인덱스. 지정이 없거나 범위 밖이면 루프의 첫 프레임.</summary>
    public int IdleIndex =>
        IdleFrameIndex is int idle && idle >= 0 && idle < FrameCount ? idle : FirstLoopIndex;

    public bool IsInLoop(int index) =>
        LoopFrames is null ? index >= 0 && index < FrameCount : LoopFrames.Contains(index);

    /// <summary>
    /// 현재 인덱스 다음에 보여줄 루프 프레임. 현재가 루프 밖(고정 프레임·복귀 프레임 등)이면 루프의 첫 프레임.
